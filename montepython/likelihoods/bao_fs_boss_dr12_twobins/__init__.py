...
class bao_fs_boss_dr12_twobins(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        for experiment in conflicting_experiments:
            if experiment in data.experiments:
                raise io_mp.LikelihoodError(
                    'bao_fs_boss_dr12_twobins reports conflicting BAO measurments from: %s' %(experiment))

        # define arrays for values of z and data points
        self.z = np.array([], 'float64')
        self.DM_rdfid_by_rd_in_Mpc = np.array([], 'float64')
        self.DH_rdfid_by_rd_in_Mpc = np.array([], 'float64')
        ...
        with open(os.path.join(self.data_directory, self.data_file), 'r') as filein:
                ...
                if line.strip() and line.find('#') == -1:
                    ...
                    if this_line[2] == 'DM_over_rs':
                        ...
                        self.DM_rdfid_by_rd_in_Mpc = np.append(
                            self.DM_rdfid_by_rd_in_Mpc, float(this_line[1]))
		    ...
		    #D_H(zeff) = c/H(zeff), where c is the speed of light and H(z) is the expansion rate.
                        ...
                        self.DH_rdfid_by_rd_in_Mpc = np.append(
                            self.DH_rdfid_by_rd_in_Mpc, float(this_line[1]))
                    # load f * sigma8
    ...
    def loglkl(self, cosmo, data):
        ...
        for i in range(self.num_bins):
            ...
            theo_DM_rdfid_by_rd_in_Mpc = DM_at_z / rd
            theo_DH_rdfid_by_rd_in_Mpc = (1./ H_at_z)/rd
            ...
            DH_diff = theo_DH_rdfid_by_rd_in_Mpc - self.DH_rdfid_by_rd_in_Mpc[i]
